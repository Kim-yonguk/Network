...
var userList=[];
var usernames = []; // 이 채팅 서버에 현재 접속한 사용자명을 저장할 변수
var rooms = ['room1', 'room2']; // 현재 채팅에서 사용가능한 룸 목록을 저장할 변수
...
io.sockets.on('connection', function (socket) {
  var nickname;
   ...
   socket.on('adduser', function(username){
    ...
    userList.push(username);
      ...
      socket.emit('updatechat', '※채널입장※ ', 'you have connected to room1');

      // 사용자가 채팅 서버에 추가되었다는 메시지를 해당 룸에만 브로드캐스팅한다.
      socket.broadcast.to('room1').emit('updatechat', '※알림※ ', username + ' has connected to this room');
  ...
  socket.on('changename', function(username){
   ...
   socket.on('switchRoom', function(newroom){
      ...
      socket.emit('updatechat', '※채널입장※ ', 'you have connected to '+ newroom);

       // 이전 룸에 대해, 사용자가 떠났단느 메시지를 브로드캐스팅한다.
      socket.broadcast.to(socket.room).emit('updatechat', '※알림※ ', socket.username+' has left this room');

      // 소켓 세션의 룸이름을 갱신한다.
      socket.room = newroom;

      socket.broadcast.to(newroom).emit('updatechat', '※알림※ ', socket.username+' has joined this room');
   ...
   socket.on('disconnect', function(){
      ...
      socket.broadcast.emit('updatechat', '※알림※ ', socket.username + ' has disconnected');
